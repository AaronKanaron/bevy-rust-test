...
fn player_movement(
    ...
    /*- Movement difference -*/
    let diff:f32 = 1.; //0.003 player.speed * TILESIZE * time.delta_seconds()
 
    /*- Handle movement -*/
    if keyboard.any_pressed(UP)    { y_delta += diff; }
    ...
    if x_delta != 0.0 || y_delta != 0.0 {
        ...
        let y_def = y_delta / magnitude / MOVEMENT_DIVIDER * player.speed * TILESIZE * 10.;
        let x_def = x_delta / magnitude / MOVEMENT_DIVIDER * player.speed * (TILESIZE * 10.);
